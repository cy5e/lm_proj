...
class LandmarkGANDataset(GANDataset):
    def __init__(self, batch_size, last_training_class_index, reverse_channels, num_of_gpus, gen_batches):
        super(LandmarkGANDataset, self).__init__(batch_size, last_training_class_index, reverse_channels, num_of_gpus,
                                                  gen_batches)


    def load_dataset(self, gan_training_index):
        DIR_DATASET = '/Users/chienyi/Git/cs231n-proj/data/train'
        DIR_DATALIST = '/Users/chienyi/Git/cs231n-proj/data'
        import matplotlib.image as mpimg

        x_val = []
        x_tmp = []
        with open(DIR_DATALIST+'/'+'sample_val.txt') as f:
            for i, line in enumerate(f):
                line = line.strip()
                filepath = DIR_DATASET + '/' + line
                x_tmp.append(mpimg.imread(filepath))

                if (i + 1) % 2 == 0:
                    x_val.append(np.array(x_tmp))
                    x_tmp = []
        x_val = np.array(x_val)
        x_val = x_val / np.max(x_val)
        print('x_val shape is: ', x_val.shape)
        print('x_val max is:', np.max(x_val))


        x_test = []
        x_tmp = []
        with open(DIR_DATALIST+'/'+'sample_test.txt') as f:
            for i, line in enumerate(f):
                line = line.strip()
                filepath = DIR_DATASET + '/' + line
                x_tmp.append(mpimg.imread(filepath))
                ...
                if (i + 1) % 2 == 0:
                    x_test.append(np.array(x_tmp))
                    x_tmp = []
        x_test = np.array(x_test)
        #x_test = np.swapaxes(x_test, 0, 1)
        x_test = x_test / np.max(x_test)
        print('x_test shape is: ', x_test.shape)
        print('x_test max is:', np.max(x_test))


        x_train = []
        x_tmp = []
        with open(DIR_DATALIST+'/'+'sample_train.txt') as f:
            for i, line in enumerate(f):
                line = line.strip()
                filepath = DIR_DATASET + '/' + line
                x_tmp.append(mpimg.imread(filepath))
                ...
                if (i + 1) % 16 == 0:
                    x_train.append(np.array(x_tmp))
                    x_tmp = []
        x_train = np.array(x_train)
        x_train = x_train / np.max(x_train)
        print('x_train shape is: ', x_train.shape)
        print('x_train max is:', np.max(x_train))




        self.x = np.concatenate((x_train, x_test, x_val), axis=1)
